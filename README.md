...
flowchart TD
    subgraph Mock_Mode ["모드 1: Mock 모드 (Expo Go)"]
        direction TB
        A1["Expo Go 앱"] --> B1["Mock Kakao Adapter"]
    ...
    subgraph Live_Mode ["모드 2: Live 모드 (Development Build)"]
        direction TB
        A2["Custom Dev Client"] --> B2["@react-native-kakao SDK"]
        B2 --> C2["실제 카카오톡 / 카카오 계정 로그인"]
        C2 --> D2["백엔드 API 서버 연동"]
    end
```
...
> 💡 **왜 Mock 모드를 사용하나요?**  
> **Expo Go** 클라이언트는 사전 빌드된 샌드박스 앱으로, 서드파티 네이티브 모듈(예: Kakao Native SDK)의 바이너리가 포함되어 있지 않습니다.  
> 따라서 별도의 Xcode/Android Studio 빌드나 백엔드 서버 없이도 Expo Go 환경에서 온보딩, 약관 동의, 프로필 설정 및 에러 시나리오를 즉시 테스트할 수 있도록 **In-Memory Mock 모드**를 지원합니다.
...
| :--- | :--- | :--- |
| `NEW_SIGNUP_USER` *(기본값)* | 신규 가입자 | 카카오 로그인 탭 시 `SIGN_UP_REQUIRED`가 반환되어 **약관 동의 → 닉네임 설정/중복확인 → 접근 권한 안내 → 이동 방식 선택 → 온보딩 완료** 전체 회원가입 플로우를 테스트합니다. |
...
실제 카카오톡 앱 간편 로그인, 카카오 웹 인증창 및 백엔드 API와의 실시간 통신을 검증할 때는 **Expo Development Build**를 생성해야 합니다.

#### 1. 사전 준비 (Kakao Developers 설정)
...
네이티브 카카오 SDK가 링크된 standalone 바이너리를 빌드하여 시뮬레이터/에뮬레이터에 설치합니다.
...
| :--- | :--- | :---: | :--- |
| `EXPO_PUBLIC_AUTH_MODE` | `live` / `mock` | 선택 | `mock` (In-Memory 모의 인증) 또는 `live` (실제 카카오 SDK & BE) 설정 |
...
| `EXPO_PUBLIC_KAKAO_NATIVE_APP_KEY` | `706f...` | Live 시 필수 | Kakao Developers 콘솔에서 발급받은 Native App Key |
