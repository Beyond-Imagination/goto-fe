...
export interface MockNaverAdapterOptions {
  readonly persona?: PersonaDefinition;
  readonly simulatedDelayMs?: number;
}

export function createMockNaverAdapter(options: MockNaverAdapterOptions = {}): SocialLoginAdapter {
  ...
  return {
    async login(): Promise<ProviderCredential> {
      ...
      return {
        provider: 'NAVER',
        providerAccessToken: `mock-naver-token-${persona.id}-${Date.now()}`,
      };
    },
  };
}
